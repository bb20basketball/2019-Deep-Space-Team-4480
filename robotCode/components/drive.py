...
class Drive(object):
    ...
    def tapeDrive(self, posX, posY):
        ...
        if self.sensors.seekTape() == SensorType.OnTarget or self.sensors.seekTape() == SensorType.Nothing:
            self.masterDrive(posX, posY)
        elif self.sensors.seekTape() == SensorType.SlightLeft:
            # Tape is slightly to the left of target
            self.masterDrive(posX, posY+adjustmentSpeed)
        elif self.sensors.seekTape() == SensorType.SlightRight:
            # Tape is slightly to the right of target
            self.masterDrive(posX, posY-adjustmentSpeed)
        elif self.sensors.seekTape() == SensorType.WayLeft:
            # Tape is way to the left of target
            self.masterDrive(posX, posY+majorAdjustmentSpeed)
        elif self.sensors.seekTape() == SensorType.WayRight:
